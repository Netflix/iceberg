...
public class TypeUtil {
  ...
  public static Schema select(Schema schema, Set<Integer> fieldIds) {
    ...
    return new Schema(
        result != null ? result.asStructType().fields() : ImmutableList.of(),
        schema.getAliases()
    );
  }
  ...
  public static Schema join(Schema left, Schema right) {
    if (right.columns().isEmpty()) {
      return left;
    } else if (left.columns().isEmpty()) {
      return right;
    }
  ...
  public static Schema join(Schema left, Types.StructType right) {
    if (right.fields().isEmpty()) {
      return left;
    } else if (left.columns().isEmpty()) {
      return new Schema(right.fields());
    }

    List<Types.NestedField> joinedColumns = Lists.newArrayList();
    joinedColumns.addAll(left.columns());
    joinedColumns.addAll(right.fields());
    return new Schema(joinedColumns);
  }

  public static Types.StructType join(Types.StructType left, Types.StructType right) {
    if (right.fields().isEmpty()) {
      return left;
    } else if (left.fields().isEmpty()) {
      return right;
    }

    List<Types.NestedField> joinedColumns = Lists.newArrayList();
    joinedColumns.addAll(left.fields());
    joinedColumns.addAll(right.fields());
    return Types.StructType.of(joinedColumns);
  }

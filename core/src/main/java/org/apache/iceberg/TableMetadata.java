...
public class TableMetadata {
  ...
  public TableMetadata removeReservedProperties(Predicate<String> isPropertyReserved) {
    ...
    Map<String, String> newProperties = builder.build();
  ...
  public TableMetadata withReservedProperties(Map<String, String> reservedProperties) {
    ...
    properties.entrySet().stream()
        .filter(entry -> !reservedProperties.containsKey(entry.getKey()))
        .forEach(newProperties::put);
    newProperties.putAll(reservedProperties);

    return new TableMetadata(file, uuid, location,
        System.currentTimeMillis(), lastColumnId, schema, defaultSpecId, specs, newProperties.build(),
        currentSnapshotId, snapshots, snapshotLog);
  }

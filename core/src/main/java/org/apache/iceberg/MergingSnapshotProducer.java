...
abstract class MergingSnapshotProducer<ThisT> extends SnapshotProducer<ThisT> {
  ...
  private final List<ManifestFile> manifestsToRewrite = Lists.newArrayList();
  private final Queue<ManifestFile> appendManifests = new ConcurrentLinkedQueue<>();
  ...
  protected void add(ManifestFile manifest) {
    ...
    manifestsToRewrite.add(manifest);
  }

  private void rewriteAppendedManifests() {
    Tasks.foreach(manifestsToRewrite)
        .throwFailureWhenFinished()
        .executeWith(ThreadPools.getWorkerPool())
        .retry(4)
        .run(manifest -> {
          SnapshotSummary.Builder manifestSummaryBuilder = SnapshotSummary.builder();
          ...
          try (ManifestReader reader = ManifestReader.read(
            ...
            appendManifests.add(ManifestWriter.copyAppendManifest(
                reader, manifestPath(manifestCount.getAndIncrement()), snapshotId(), manifestSummaryBuilder));
          } catch (IOException e) {
            throw new RuntimeIOException(e, "Failed to close manifest: %s", manifest);
          }

          synchronized (appendedManifestsSummary) {
            appendedManifestsSummary.merge(manifestSummaryBuilder);
          }
        });

    // clear the pending rewrite list to avoid rewriting twice
    manifestsToRewrite.clear();
  }

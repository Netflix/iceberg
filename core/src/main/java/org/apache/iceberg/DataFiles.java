...
public class DataFiles {
  ...
  public static DataFile fromManifest(ManifestFile manifest) {
    long fileCount = manifest.addedFilesCount() == null || manifest.existingFilesCount() == null ?
        1 : manifest.addedFilesCount() + manifest.existingFilesCount();
    return new GenericDataFile(manifest.path(), FileFormat.AVRO, fileCount, manifest.length());
  }

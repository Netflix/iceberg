...
public abstract class BaseMetastoreTableOperations implements TableOperations {
  ...
  public TableMetadata refresh() {
    try {
      ...
      if (!(e instanceof NotIcebergException)) {
  ...
  protected void refreshFromMetadataLocation(String newLocation, Predicate<Exception> shouldRetry,
                                             int numRetries) {
    refreshFromMetadataLocation(newLocation, shouldRetry, numRetries, null);
  }

  protected void refreshFromMetadataLocation(String newLocation, Predicate<Exception> shouldRetry,
                                             int numRetries, Function<TableMetadata, TableMetadata> updateMetadata) {
...
//            "Table UUID does not match: current=%s != refreshed=%s", currentMetadata.uuid(), newUUID);
      ...
      if (updateMetadata != null) {
        this.currentMetadata = updateMetadata.apply(newMetadata.get());
      } else {
